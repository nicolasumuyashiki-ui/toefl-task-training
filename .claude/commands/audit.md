...
- **(C) RDL の問題数が語数ルール（15-50語→2問 / 51-150語→3問）と不一致**（8 パッセージ）— **保留**。
  方針が決まるまでは報告を継続してよい。
- **(F) Academic Passage の後半（Harder）パッセージが 4 段落**（practice-1/2/3/4/5/7）— **リサーチ済み・判断待ち**。
  2026-08-24 の調査結果: ETS 公式は「約 200 語・5 問」のみ規定で、**段落数の規定は存在しない**。
  むしろ ETS のタスク説明は "how ideas are logically connected across sentences **and paragraphs**" と
  複数段落を前提にしている。`new-academic.md` の「2-3 段落」は**プロジェクト独自の慣習**であり公式要件ではない。
