...
4. ~~**正解分布のバランス**: A/B/C/D が各 2-3 個程度に均等か~~ — **2026-08-24 に監査項目から除外**（下記「監査対象外」E 参照）。検出・報告しない。
...
## 監査対象外（判断済み・以後この項目は検出も報告もしない）
一度オーナー判断が下りた事項を毎朝再掲すると、本当に新しい異常が埋もれる。下記は**既に判断済み**なので、
検出ロジックから外すこと（「見落とし」ではなく「意図的な仕様」である）。

- **(1) `reading/academic/practice-5.html:391` / `practice-5-answers.html:364` の `According to paragraph 4, ...`**
  （2026-08-24 判断）Factual Detail 設問だが旧形式の paragraph 参照が残っている。**修正しない** —
  既存問題の設問文に直接手を入れることになるため。以後この 1 箇所を指摘しない。
  ※ 新規作成時は従来どおり `According to the passage, ...` を使うこと（`new-academic.md` の規則は有効）。
- **(D) RDL のパッセージ語数が 150 語上限を超えること**（2026-08-24 判断）
  対象例: `reading/rdl/` practice-1 / 2 / 3 / 4 / 8 / 10。**許容**。以後 RDL の語数上限超過は検出しない。
- **(E) 正解文字（A/B/C/D）の分布の偏り**（2026-08-24 判断）
  例: RDL 全 51 問で B が 52.9%、Academic practice-6 で D が 1 問のみ。**許容**。
  正誤・難易度に実害がないため、以後タスク横断の正解分布チェックは行わない（チェック項目 A-4 を無効化済み）。

- **(F) Academic Passage の段落数が 4 段落**（practice-1/2/3/4/5/7 の Harder パッセージ）（2026-08-24 判断）
  リサーチの結果、ETS 公式は「約 200 語・5 問」のみを規定し**段落数の規定は存在しない**ことを確認
  （公式のタスク説明は "how ideas are logically connected across sentences **and paragraphs**" と
  複数段落を前提にしている）。`new-academic.md` の「2-3 段落」は公式要件ではなくプロジェクト独自の慣習だったため、
  **仕様書側を 2-4 段落へ更新して既存 6 ファイルは現状維持**とした（B案）。
  以後、4 段落を逸脱として指摘しない。**5 段落以上のみ**を指摘対象とする。

### 判断保留（次回以降も報告してよい）
- **(C) RDL の問題数が語数ルール（15-50語→2問 / 51-150語→3問）と不一致**（8 パッセージ）— **保留**。
  方針が決まるまでは報告を継続してよい。
