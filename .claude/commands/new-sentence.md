...
> 問題数（10問固定）・制限時間（約7分/6分50秒）・Word Bank語数（5〜7語）は ETS 公式情報と
> 複数の第三者ソース（Test Resources, Magoosh, goarno.io 等）で一致確認済み。全問オールオアナッシング採点
> （1問1点、部分点なし）。
...
## 指定
$ARGUMENTS
- 例: 空 → 10問固定（ETS 2026仕様）、トピックランダム
- 例: `travel` → 10問、旅行トピック中心
- ※ 問題数は ETS 公式仕様により10問固定。指定できるのはトピックのみ。

## 出力ファイル
1. `writing/sentence/practice-{N}.html` — ドラッグ＆ドロップ形式HTML
2. `writing/sentence/practice-{N}.md` — 問題文＋Answer Key

## Build a Sentence 問題仕様 (v2.1)

### 概要
- 10問固定（ETS 2026 仕様）
- 会話形式: 相手の発話への返答を並べ替えで作成
- ドラッグ＆ドロップで単語をBlankに配置
- 制限時間: 7分（420秒／実測6分50秒）固定・10問（ETS 2026 仕様）
...
### 段階的配置ルール（必須）
- Basic問題を**序盤**（Q1〜Q3付近）に配置
- Standard問題を**中盤**（Q4〜Q7付近）に配置
- Advanced問題を**後半**（Q8〜Q10付近）に配置
- 段階的に難しくなる構成にすること
...
### 作成後
1. HTML＋MDファイルを作成
2. 検証コード v2.1 実行（シャッフル、Blank数5-7、難易度分布、文構造バリエーション、段階的配置）
