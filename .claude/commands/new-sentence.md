...
- 制限時間: 7分（420秒）固定・10問（ETS 2026 仕様）
...
## 🆕 prompt を改訂したら、解答ページも同じ新しい問題文にする（2026-07-25 改定）

既存問題の **prompt（会話文）を改訂**する場合、**採点対象（`blanks`/`answer`）を1文字も変えないなら、旧版アーカイブも
当時版の出し分けも一切不要**。練習ページと解答ページの**両方をそのまま新しい prompt に差し替える**だけでよい。

1. 練習ページ・解答ページの当該設問の `prompt` を新しい文言に差し替える（**両方**を必ず揃える）。
2. 改訂前に取り組んだ生徒が「回答を見る」を開いても、**現行の prompt が表示される**状態にする。
   採点対象の英文が不変なので、当時の正誤・スコアはそのまま有効。
3. 旧 prompt を記録として残したい場合のみ、データに `promptLegacy` を置いてよい。**画面には出さない**。
4. **禁止**: `answer`/`blanks`（並べ替える語群・正解）を変えること。変える場合は CTW 同様の旧版アーカイブ＋
   era境界が必須（CLAUDE.md「旧版アーカイブ必須」参照）。

> ⚠️ **廃止**: #165 で入れた `PROMPT_REV` / `?rev=legacy` による「当時の問題文で見返す／解き直す」導線は、
> 復習が著しくやりにくいという理由で **#194 以降で廃止**した（`writing/sentence/practice-{2,6}*.html` から
> 実装ごと削除済み）。今後 prompt を改訂しても、この仕組みを復活させないこと。

> ⚠️ 会話整合（§K）修正で prompt を直すときも上記に従う。生徒の取り組み履歴・スコアは**表示のみ**が変わり、
> サーバ ANSWERS・`training_score_*` には一切書き込まない（読み取り＋表示キャッシュのみ）。
