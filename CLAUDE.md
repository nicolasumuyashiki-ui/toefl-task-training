...
## 問題を改修するときは旧版アーカイブ必須（再発防止・最重要・2026-07-12 制定）
**既存 practice の問題文・正解を書き換えると、それ以前の受講記録（サーバに残る回答）が現行ページ上で
意味を失い「復習できない」状態になる**（CTW 全面リビルド #125 で実際に発生 → 面談クレームの主因の一つ）。
既存問題の内容（問題文・選択肢・正解・空所）を変更する場合は、**同じコミットで必ず**:
1. 改修前のファイル一式を `<section>/<task>/legacy/`（2世代目以降は `legacy-YYYYMMDD/`）へ退避する。
   参照実装: `reading/ctw/legacy/`（パスを1階層深く書き換え・保存キーを `ctwL_*` 等に隔離・
   `Api.saveAnswers`/`Auth.completeSet` を除去した**復習専用モード**・旧版バナー＋「▶ 復習する」ボタン・
   サーバに残る当時の回答を旧版ページ上へ復元表示・`?fromAdmin=1` 対応）。
...
4. **問題文（prompt・会話文・設問文）だけを改訂し、`answer`/`blanks`（＝生徒が入力・選択する対象）を
   1文字も変えない場合は、旧版アーカイブも当時版の出し分けも一切不要**（2026-07-25 方針確定）。
   練習ページと解答ページの両方を**そのまま新しい問題文に差し替える**。改訂前に取り組んだ生徒が「回答を見る」を
   開いても、**現行の問題文**が表示される状態にすること。正誤・スコアは入力対象が不変なのでそのまま有効。
   - **当時の問題文を出し分けてはいけない**。以前 #165 で導入した `PROMPT_REV` / `?rev=legacy` による
     「当時の問題文で見返す／解き直す」導線は、復習が著しくやりにくくなるため**廃止した**（#194 以降）。
   - 旧 prompt を記録として残したい場合のみ、データに `promptLegacy` を置いてよい。**画面には出さない**。
   - **サーバ ANSWERS / `training_score_*` には一切書き込まない**（表示の差し替えのみ）。
...
- **切替境界 `SCORE_CUTOFF = '2026-06-29T15:00:00Z'`**（= 6/30 0:00 JST）を `js/history-sync.js`・`my-score.html`・`js/student-history.js`（スコア救済表示）・`admin/index.html`（管理画面のスコア表示）の**4箇所**に持つ。変更時は必ず全部そろえる。
...
- **据え置き（前田・矢口）との関係**: BANDS フロアは「新しい取り組みをするまで」現在値を保持する床として今も有効。re-engage したセクションだけ最新に追従する（＝全員・今後の取り組みからの方針と一致）。**矢口さまの Reading 4.5 固定は本方針と矛盾するため実行しない**。
  専用関数 `holdYaguBands`（旧称 holdYaguReading）は 2026-07-25 の監査で GAS から削除した。引数なしで一発実行でき、
  誤ってドロップダウンから選ぶと本方針に反する固定値が書き込まれるため。矢口さまのバンドは通常の `saveBands`
  （`_mergeBands_` が最高値をマージ＝床を下げない）で従来どおり維持される。
  **今後も「特定顧客のスコアを直接書き換える関数」を GAS に置かないこと。**
